...
open GPERF, ">CSSPropertyNames.cpp" || die "Could not open CSSPropertyNames.cpp for writing";
print GPERF << "EOF";
...
static constexpr std::pair<frozen::string, CSSPropertyID> propertyNames[] = {
        { "invalid",  CSSPropertyInvalid },
...
static constexpr auto s_NamedProperties = frozen::make_unordered_map(propertyNames);

const CSSPropertyID findProperty(const char* str, int len) {
    return s_NamedProperties.find(frozen::string(str, len))->second;
}
...
std::vector<std::string> CSSProperty::aliasesForProperty(CSSPropertyID id) {
    switch (id) {
EOF
...
const std::atomic<std::string>& getPropertyNameAtomString(CSSPropertyID id);
std::string getPropertyNameString(CSSPropertyID id);
std::string getJSPropertyName(CSSPropertyID);
